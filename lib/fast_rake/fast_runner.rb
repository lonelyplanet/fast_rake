...
class FastRake::FastRunner
  ...
  def distance_of_time_to_now(time)
    ...
    format('%02dm %02ds', total_minutes, seconds_in_last_minute)
  end
  ...
  def start_some_children
    while @children.length < @process_count && !@tasks.empty?
  ...
  def wait_for_tasks_to_finish
    loop do # When there are no more child processes wait2 raises Errno::ECHILD
